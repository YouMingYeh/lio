...
const fileEventHandler = async (
  ...
  if (event.message.type !== "file") return;
  const lineApiClient = new LINEAPIClient(client, blobClient);

  const repository = new Repository();
  const message = event.message as webhook.FileMessageContent;
  ...
  const fileParts = [
    {
      type: "text",
      text: "(This is an file, please repond it but DO NOT EXPOSE YOUR THINKING STEPS)",
    },
    {
      type: "file",
      data: url,
